...
//
//   sessions/{CODE}/
//     name: "Cardiology Grand Rounds"           <- presenter-chosen session title
...
// ---------- efficient vote watching ----------
// Watches a votes node with child_added/changed/removed listeners instead of
// on("value"). on("value") re-downloads the WHOLE votes list on every vote
// (quadratic bandwidth — the main drain on the Firebase free tier); child
// listeners download only the ~50-byte delta. Same real-time speed.
// Returns { votes, off() }: `votes` is a live map you can pass to
// pollCountVotes. onChange is debounced (40ms) so a burst of incoming votes
// (e.g. initial load of 200 existing votes) triggers one render, not 200.
function pollWatchVotes(ref, onChange) {
  const votes = {};
  let timer = null;
  const notify = () => {
    if (timer) return;
    timer = setTimeout(() => { timer = null; onChange(votes); }, 40);
  };
  const put = (s) => { votes[s.key] = s.val(); notify(); };
  const drop = (s) => { delete votes[s.key]; notify(); };
  ref.on("child_added", put);
  ref.on("child_changed", put);
  ref.on("child_removed", drop);
  // fires once initial children have loaded, even if there are none
  ref.once("value", (s) => { if (!s.hasChildren()) notify(); });
  return {
    votes,
    off() {
      if (timer) { clearTimeout(timer); timer = null; }
      ref.off("child_added", put);
      ref.off("child_changed", put);
      ref.off("child_removed", drop);
    }
  };
}
